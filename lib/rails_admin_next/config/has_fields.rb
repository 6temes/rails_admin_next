    ...
    module HasFields
      ...
      def field(name, type = nil, add_to_section = true, &block)
        ...
        if field.nil? && type.nil?
          field = (_fields << RailsAdminNext::Config::Fields::Types.load(:string).new(self, name, nil)).last
        ...
        # one found in default stack
          ...
          if field
            ...
            field = _fields[_fields.index(field)] = RailsAdminNext::Config::Fields::Types.load(type).new(self, name, properties)
          else
            properties = abstract_model.properties.detect { |p| name == p.name }
            field = (_fields << RailsAdminNext::Config::Fields::Types.load(type).new(self, name, properties)).last
          end
        end
      ...
      def configure(name, type = nil, &)
        [*name].each { |field_name| field(field_name, type, false, &) }
      end
      ...
      def include_fields(*field_names, &)
        if field_names.empty?
          _fields.select { |f| f.instance_eval(&) }.each do |f|
            next if f.defined
      ...
      def _fields(readonly = false)
        ...
        if instance_of?(RailsAdminNext::Config::Sections::Base)
          @_ro_fields = @_fields = RailsAdminNext::Config::Fields.factory(self)
        else
          # parent is RailsAdminNext::Config::Model, recursion is on Section's classes
