...
module RailsAdminNext
  module Config
    module Fields
      class Association < RailsAdminNext::Config::Fields::Base
        ...
        register_instance_option :pretty_value do
          ...
          [value].flatten.select(&:present?).collect do |associated|
            amc = polymorphic? ? RailsAdminNext.config(associated) : associated_model_config # perf optimization for non-polymorphic associations
        ...
        # Keyed by the association's name (not its accessor key), so it resolves via
        # human_attribute_name like any other attribute label.
        ...
        def associated_model_config
          @associated_model_config ||= RailsAdminNext.config(association.klass)
        end
        ...
        def associated_model_limit
          RailsAdminNext.config.default_associated_collection_limit
        end
        ...
        def format_key(key)
          if key.is_a?(Array)
            RailsAdminNext.config.composite_keys_serializer.serialize(key)
          else
            key
          end
        end
