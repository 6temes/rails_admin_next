...
RailsAdminNext::Config::Fields.register_factory do |parent, properties, fields|
  parent.abstract_model.associations.filter { |a| Array(a.foreign_key).include?(properties.name) && %i[belongs_to has_and_belongs_to_many].include?(a.type) }.each do |association|
    field = RailsAdminNext::Config::Fields::Types.load(association.field_type).new(parent, association.name, association)
    fields << field
    ...
    parent.abstract_model.properties.select { |p| possible_field_names.include? p.name }.each do |column|
      ...
      child_field ||= RailsAdminNext::Config::Fields.default_factory.call(parent, column, fields)
