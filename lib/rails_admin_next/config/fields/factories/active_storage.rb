...
RailsAdminNext::Config::Fields.register_factory do |parent, properties, fields|
  if defined?(::ActiveStorage) && properties.try(:association?) && (match = /\A(.+)_attachments?\Z/.match properties.name) && properties.klass.to_s == "ActiveStorage::Attachment"
    name = match[1]
    field = RailsAdminNext::Config::Fields::Types.load(
    ...
    children_fields = associations.map do |child_name|
      ...
      child_field = fields.detect { |f| f.name == child_name } || RailsAdminNext::Config::Fields.default_factory.call(parent, child_association, fields)
