...
require "rails_admin_next/config/fields/singular_association"

module RailsAdminNext
  module Config
    module Fields
      module Types
        class BelongsToAssociation < RailsAdminNext::Config::Fields::SingularAssociation
          ...
          def parse_input(params)
            ...
            association.foreign_key.zip(RailsAdminNext.config.composite_keys_serializer.deserialize(params.delete(method_name))).each do |key, value|
              params[key] = value
            end
          end
        end
