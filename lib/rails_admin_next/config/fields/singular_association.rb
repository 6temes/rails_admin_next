...
require "rails_admin_next/config/fields/association"
    ...
    module Fields
      class SingularAssociation < Association
        ...
        def parse_input(params)
          # A singular association's nested params are a single record hash; a crafted request can
          # send an array instead, on which `[:id]` would raise. Only parse the expected shape.
          nested = params[method_name]
          return unless nested_form && nested.respond_to?(:key?) && nested[:id].present?

          ids = associated_model_config.abstract_model.parse_id(nested[:id])
