...
require "rails_admin_next/config/sections"
require "rails_admin_next/config/actions"
...
module RailsAdminNext
  module Config
    ...
    class Model
      ...
      include RailsAdminNext::Config::Sections
      ...
      def initialize(entity)
        ...
        @abstract_model =
          ...
          when RailsAdminNext::AbstractModel
            entity
          when Class, String
            RailsAdminNext::AbstractModel.new(entity)
          when Symbol
            RailsAdminNext::AbstractModel.new(entity.to_s)
          else
            RailsAdminNext::AbstractModel.new(entity.class)
          end

        @groups = [RailsAdminNext::Config::Fields::Group.new(self, :default).tap { |g| g.label { I18n.translate("admin.form.basic_info") } }]
      end

      def excluded?
        return @excluded if defined?(@excluded)

        @excluded = !RailsAdminNext::AbstractModel.all.collect(&:model_name).include?(abstract_model.try(:model_name))
      end
      ...
      # Logical icon name (see RailsAdminNext::Icons::GLYPHS) rendered as inline SVG
      # next to the model in the sidebar; nil for no icon.
      ...
      def respond_to_missing?(method_name, include_private = false)
        send(:base).respond_to?(method_name, include_private) || super
      end
    end
  end
end
