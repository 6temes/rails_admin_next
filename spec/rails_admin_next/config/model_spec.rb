...
RSpec.describe RailsAdminNext::Config::Model do
  describe "#excluded?" do
    ...
    it "returns false when included, true otherwise" do
      allow(RailsAdminNext::AbstractModel).to receive(:all).and_call_original

      player_config = RailsAdminNext.config(Player)
      expect(player_config.excluded?).to be_truthy
      expect(RailsAdminNext::AbstractModel).to have_received(:all).once
      # Calling a second time uses the cached value.
      expect(player_config.excluded?).to be_truthy
      expect(RailsAdminNext::AbstractModel).to have_received(:all).once

      comment_config = RailsAdminNext.config(Comment)
      expect(comment_config.excluded?).to be_falsey
      expect(RailsAdminNext::AbstractModel).to have_received(:all).twice
      # Calling a second time uses the cached value.
      expect(comment_config.excluded?).to be_falsey
      expect(RailsAdminNext::AbstractModel).to have_received(:all).twice
    end
  ...
  describe "#object_label" do
    ...
    it "sends object_label_method to binding[:object]" do
      ...
      expect(RailsAdminNext.config(Comment).with(object: c).object_label).to eq("test")
    end

    context "when the object_label_method is blank" do
      it "uses the rails admin default" do
        c = Comment.create(content: "", id: "1")
        expect(RailsAdminNext.config(Comment).with(object: c).object_label).to eq("Comment #1")
      end
    end
  end
  ...
  describe "#object_label_method" do
    it "is first of Config.label_methods if found as a column on model, or :rails_admin_default_object_label_method" do
      expect(RailsAdminNext.config(Comment).object_label_method).to eq(:rails_admin_default_object_label_method)
      expect(RailsAdminNext.config(Division).object_label_method).to eq(:name)
    end
  end

  describe "#label" do
    it "is pretty" do
      expect(RailsAdminNext.config(Comment).label).to eq("Comment")
    end
  end

  describe "#label_plural" do
    it "is pretty" do
      expect(RailsAdminNext.config(Comment).label_plural).to eq("Comments")
    end
    ...
    context "when using i18n as label source" do
      ...
      context "and the locale uses a specific pluralization rule" do
        ...
        it "always uses :other as pluralization key" do
          expect(RailsAdminNext.config(Comment).label_plural).to eq("other")
        end
      end
    end
  end
  ...
  describe "#weight" do
    it "is 0" do
      expect(RailsAdminNext.config(Comment).weight).to eq(0)
    end
  end

  describe "#parent" do
    it "is nil for ActiveRecord::Base inherited models" do
      expect(RailsAdminNext.config(Comment).parent).to be_nil
    end

    it "is parent model otherwise" do
      expect(RailsAdminNext.config(Hardball).parent).to eq(Ball)
    end
  end

  describe "#navigation_label" do
    it "is nil if parent module is Object" do
      expect(RailsAdminNext.config(Comment).navigation_label).to be_nil
    end

    it "is parent module otherwise" do
      expect(RailsAdminNext.config(Cms::BasicPage).navigation_label).to eq("Cms")
    end
  end
  ...
  describe "#last_created_at", active_record: true do
    ...
    it "allow customization" do
      expect(RailsAdminNext.config(Team).last_created_at.to_date).to eq 1.day.ago.to_date
    end
  end
end
