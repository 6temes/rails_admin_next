# frozen_string_literal: true

require "spec_helper"

RSpec.describe RailsAdminNext::Config::Fields::Types::ActiveRecordEnum, active_record: true do
  it_behaves_like "a generic field type", :string_enum_field

  describe "#pretty_value" do
    context "when column name is format" do
      before do
        stub_const("FormatAsEnum", Class.new(FieldTest) do
          enum :format, {Text: "txt", Markdown: "md"}
        end)
      end
      let(:field) do
        RailsAdminNext.config(FormatAsEnum).fields.detect do |f|
          f.name == :format
        end.with(object: FormatAsEnum.new(format: "md"))
      end

      it "does not break" do
        expect(field.pretty_value).to eq "Markdown"
      end
    end
  end
end
