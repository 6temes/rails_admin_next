...
RSpec.describe RailsAdminNext::Config::Fields::Types::Date do
  it_behaves_like "a generic field type", :date_field, :date

  describe "#formatted_value" do
    it "gets object value" do
      field = RailsAdminNext.config(FieldTest).fields.detect do |f|
        f.name == :date_field
    ...
    it "gets default value for new objects if value is nil" do
      RailsAdminNext.config(FieldTest) do |_config|
        field :date_field do
  ...
  describe "#parse_input" do
    let(:field) { RailsAdminNext.config(FieldTest).fields.detect { |f| f.name == :date_field } }
  ...
  describe "#default_value" do
    let(:field) do
      field = RailsAdminNext.config(FieldTest).fields.detect { |f| f.name == :date_field }
