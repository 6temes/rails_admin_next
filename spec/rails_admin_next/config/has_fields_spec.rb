...
RSpec.describe RailsAdminNext::Config::HasFields do
  it "shows hidden fields when added through the DSL" do
    expect(RailsAdminNext.config(Team).fields.detect { |f| f.name == :division_id }).not_to be_visible
    ...
    expect(RailsAdminNext.config(Team).fields.detect { |f| f.name == :division_id }).to be_visible
  end
  ...
  it "does not set visibility for fields with bindings" do
    ...
    expect { RailsAdminNext.config(Team).fields.detect { |f| f.name == :division } }.not_to raise_error
    expect { RailsAdminNext.config(Team).fields.detect { |f| f.name == :division }.visible? }.to raise_error(/undefined method [`']\[\]' for nil/)
  end
  ...
  it "assigns properties to new one on overriding existing field" do
    ...
    expect(RailsAdminNext.config(Team).fields.detect { |f| f.name == :players }.properties).not_to be_nil
  end

  describe "#configure" do
    it "does not change the order of existing fields, if some field types of them are changed" do
      original_fields_order = RailsAdminNext.config(Team).fields.map(&:name)
      ...
      expect(RailsAdminNext.config(Team).fields.map(&:name)).to eql(original_fields_order)
    end

    it "allows passing multiple fields to share the same configuration" do
      target_field_names = %i[players revenue]

      original_config = RailsAdminNext.config(Team).fields.select { |field| target_field_names.include?(field.name) }
      ...
      updated_config = RailsAdminNext.config(Team).fields.select { |field| target_field_names.include?(field.name) }
      updated_config.each { |field| expect(field).to_not be_visible }
    end
  end

  describe "#_fields" do
    let(:config) { RailsAdminNext.config(Team) }
    before do
