...
RSpec.describe "RailsAdminNext::Adapters::ActiveRecord", active_record: true do
  ...
  describe "#associations" do
    it "returns Association class" do
      expect(RailsAdminNext::AbstractModel.new(Player).associations.first)
        .to be_a_kind_of RailsAdminNext::Adapters::ActiveRecord::Association
    end
  end

  describe "#properties" do
    it "returns Property class" do
      expect(RailsAdminNext::AbstractModel.new(Player).properties.first)
        .to be_a_kind_of RailsAdminNext::Adapters::ActiveRecord::Property
    end
  end

  describe "#base_class" do
    it "returns inheritance base class" do
      expect(RailsAdminNext::AbstractModel.new(Hardball).base_class).to eq Ball
    end
  end
  ...
  describe "data access methods" do
    ...
    describe "#count" do
      ...
      context "when default-scoped with select" do
        before do
          stub_const("PlayerWithDefaultScope", Class.new(Player) do
            self.table_name = "players"
            default_scope { select(:id, :name) }
          end)
        end
        let(:abstract_model) { RailsAdminNext::AbstractModel.new("PlayerWithDefaultScope") }
    ...
    describe "#all" do
      ...
      it "supports retrieval by bulk_ids with composite primary keys", composite_primary_keys: true do
        expect(RailsAdminNext::AbstractModel.new(Fanship).all(
          bulk_ids: %w[1_2 3_4]
      ...
      it "supports pagination" do
        expect(abstract_model.all(sort: "id", page: 2, per: 1).to_a).to eq(@players[-2, 1])
        expect(abstract_model.all(sort: "id", page: 1, per: 2).to_a).to eq(@players[-2, 2].reverse)
      end
  ...
  describe "#query_scope" do
    let(:abstract_model) { RailsAdminNext::AbstractModel.new("Team") }
  ...
  describe "#build_statement" do
    let(:abstract_model) { RailsAdminNext::AbstractModel.new("FieldTest") }
