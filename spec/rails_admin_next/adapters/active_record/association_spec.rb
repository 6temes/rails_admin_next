...
# Shared across the whole file: examples reference these constants directly
# (e.g. `eq ARBlog`) alongside the memoized AbstractModel instances built once
# below, so they must exist at load time rather than be re-stubbed per example.
...
RSpec.describe "RailsAdminNext::Adapters::ActiveRecord::Association", active_record: true do
  before :all do
    ...
    @blog = RailsAdminNext::AbstractModel.new(ARBlog)
    @post = RailsAdminNext::AbstractModel.new(ARPost)
    @category = RailsAdminNext::AbstractModel.new(ARCategory)
    @user = RailsAdminNext::AbstractModel.new(ARUser)
    @profile = RailsAdminNext::AbstractModel.new(ARProfile)
    @comment = RailsAdminNext::AbstractModel.new(ARComment)
  end
  ...
  describe "has_many association" do
    let(:league) { RailsAdminNext::AbstractModel.new(League) }
  ...
  describe "has_many association with not nullable foreign key" do
    let(:field_test) { RailsAdminNext::AbstractModel.new(FieldTest) }
    ...
    context "when foreign_key is passed as Symbol" do
      before do
        stub_const("FieldTestWithSymbolForeignKey", Class.new(FieldTest) do
          has_many :nested_field_tests, dependent: :destroy, inverse_of: :field_test, foreign_key: :field_test_id
        end)
      end
      let(:field_test) { RailsAdminNext::AbstractModel.new(FieldTestWithSymbolForeignKey) }
  ...
  describe "polymorphic belongs_to association" do
    before { allow(RailsAdminNext::Config).to receive(:models_pool).and_return(%w[ARBlog ARPost ARCategory ARUser ARProfile ARComment]) }
    ...
    describe "on a subclass" do
      before do
        stub_const("ARReview", Class.new(ARComment))
        allow(RailsAdminNext::Config).to receive(:models_pool).and_return(%w[ARBlog ARPost ARCategory ARUser ARProfile ARComment ARReview])
      end
      subject { RailsAdminNext::AbstractModel.new(ARReview).associations.detect { |a| a.name == :commentable } }
