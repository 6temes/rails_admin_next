...
RSpec.describe RailsAdminNext::ApplicationHelper, type: :helper do
  describe "#authorized?" do
    let(:abstract_model) { RailsAdminNext.config(FieldTest).abstract_model }
  ...
  describe "with #authorized? stubbed" do
    ...
    describe "#current_action?" do
      it "returns true if current_action, false otherwise" do
        @action = RailsAdminNext::Config::Actions.find(:index)

        expect(helper.current_action?(RailsAdminNext::Config::Actions.find(:index))).to be_truthy
        expect(helper.current_action?(RailsAdminNext::Config::Actions.find(:show))).not_to be_truthy
      end
    end
    ...
    describe "#action" do
      ...
      it "returns only visible actions, passing all bindings" do
        ...
        expect(helper.action(:test_bindings, RailsAdminNext::AbstractModel.new(Team), Team.new)).to be
        expect(helper.action(:test_bindings, RailsAdminNext::AbstractModel.new(Team), Player.new)).to be_nil
        expect(helper.action(:test_bindings, RailsAdminNext::AbstractModel.new(Player), Team.new)).to be_nil
      end
    end
    ...
    describe "#actions" do
      ...
      it "only returns visible actions, passing bindings correctly" do
        ...
        expect(helper.actions(:all, RailsAdminNext::AbstractModel.new(Team), Team.new).collect(&:custom_key)).to eq([:test_bindings])
        expect(helper.actions(:all, RailsAdminNext::AbstractModel.new(Team), Player.new).collect(&:custom_key)).to eq([])
        expect(helper.actions(:all, RailsAdminNext::AbstractModel.new(Player), Team.new).collect(&:custom_key)).to eq([])
      end
    end
    ...
    describe "#wording_for" do
      ...
      it "passes correct bindings" do
        expect(helper.wording_for(:title, :edit, RailsAdminNext::AbstractModel.new(Team), Team.new(name: "the avengers"))).to eq("Edit Team 'the avengers'")
      end

      it "defaults correct bindings" do
        @action = RailsAdminNext::Config::Actions.find :edit
        @abstract_model = RailsAdminNext::AbstractModel.new(Team)
      ...
      it "does not try to use the wrong :label_metod" do
        @abstract_model = RailsAdminNext::AbstractModel.new(Draft)
        @object = Draft.new

        expect(helper.wording_for(:link, :new, RailsAdminNext::AbstractModel.new(Team))).to eq("Add a new Team")
      end
    end

    describe "#breadcrumb" do
      it "gives us a breadcrumb" do
        @action = RailsAdminNext::Config::Actions.find(:edit, abstract_model: RailsAdminNext::AbstractModel.new(Team), object: FactoryBot.create(:team, name: "the avengers"))
    ...
    describe "#menu_for" do
      it "passes model and object as bindings and generates a menu, excluding non-get actions" do
        ...
        @action = RailsAdminNext::Config::Actions.find :show
        @abstract_model = RailsAdminNext::AbstractModel.new(Team)
      ...
      it "excludes non-get actions" do
        ...
        @action = RailsAdminNext::Config::Actions.find :dashboard
        expect(helper.menu_for(:root)).not_to match(/Dashboard/)
      end

      it "includes actions whose verbs name GET in any case" do
        RailsAdminNext.config do |config|
          config.actions do
            dashboard do
              http_methods %i[GET]
    ...
    describe "#bulk_menu" do
      it "includes all visible bulkable actions" do
        ...
        # Preload all models to prevent I18n being cleared
        ...
        result_2 = helper.bulk_menu(RailsAdminNext::AbstractModel.new(Player))
    ...
    describe "#edit_user_link" do
      ...
      context "when gravatar is disabled" do
        before { RailsAdminNext.config.show_gravatar = false }
      ...
      context "when the user is not authorized to perform edit" do
        before do
          allow_any_instance_of(RailsAdminNext::Config::Actions::Edit).to receive(:authorized?).and_return(false)
        end
