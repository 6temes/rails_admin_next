...
RSpec.describe RailsAdminNext::MainController, type: :controller do
  routes { RailsAdminNext::Engine.routes }
  ...
  before do
    controller.instance_variable_set :@action, RailsAdminNext::Config::Actions.find(:index)
  end
  ...
  describe "#get_sort_hash" do
    context "options sortable is a hash" do
      ...
      it "returns the option with no changes" do
        ...
        expect(controller.send(:get_sort_hash, RailsAdminNext.config(Player))).to eq(sort: :"team.name", sort_reverse: true)
      end
    end
    ...
    context "when default sort_by points to a field with a table reference for sortable" do
      ...
      it "returns the query referenced in the sortable" do
        expect(controller.send(:get_sort_hash, RailsAdminNext.config(Player))).to eq(sort: "teams.name", sort_reverse: true)
      end
    end
    ...
    context "with a virtual field" do
      ...
      it "returns the query referenced in the sortable" do
        expect(controller.send(:get_sort_hash, RailsAdminNext.config(Player))).to match(sort: /["`]?players["`]?\.["`]?name["`]?/, sort_reverse: true)
      end
    end
    ...
    it "works with belongs_to associations with label method virtual" do
      ...
      expect(controller.send(:get_sort_hash, RailsAdminNext.config(Category))).to eq(sort: "categories.parent_category_id", sort_reverse: true)
    end
    ...
    context "using active_record", active_record: true do
      ...
      it "gives back the local column" do
        controller.params = {sort: "team", model_name: "players"}
        expect(controller.send(:get_sort_hash, RailsAdminNext.config(Player))).to match(sort: /^["`]teams["`]\.["`]name["`]$/, sort_reverse: true)
      end
      ...
      it "quotes the table and column names it returns as :sort" do
        ...
        when "mysql2"
          expect(controller.send(:get_sort_hash, RailsAdminNext.config(Player))[:sort]).to eq "`teams`.`name`"
        else
          expect(controller.send(:get_sort_hash, RailsAdminNext.config(Player))[:sort]).to eq '"teams"."name"'
        end
      end
  ...
  describe "#list_entries called from view" do
    ...
    it "paginates" do
      expect(controller.list_entries(RailsAdminNext.config(Team), :index, nil, false).to_a.length).to eq(21)
      expect(controller.list_entries(RailsAdminNext.config(Team), :index, nil, true).to_a.length).to eq(20)
    end
  end
  ...
  describe "#list_entries called with bulk_ids" do
    ...
    it "does not paginate" do
      expect(controller.list_entries(RailsAdminNext.config(Team), :bulk_delete).to_a.length).to eq(21)
    end
  end
  ...
  describe "#action_missing" do
    it "raises error when action is not found" do
      expect(RailsAdminNext::Config::Actions).to receive(:find).and_return(nil)
  ...
  describe "#get_collection" do
    ...
    let(:model_config) { RailsAdminNext.config(Team) }
    ...
    context "on export" do
      before do
        controller.instance_variable_set :@action, RailsAdminNext::Config::Actions.find(:export)
      end
  ...
  describe "index" do
    ...
    context "when authorizing requests with pundit" do
      controller(RailsAdminNext::MainController) do
  ...
  describe "sanitize_params_for!" do
    ...
    it "allows for delete method with ActiveStorage" do
      ...
      controller.params = HashWithIndifferentAccess.new(
        "field_test" => {
          "active_storage_asset" => "test",
          "remove_active_storage_asset" => "test",
          "active_storage_assets" => "test",
          "remove_active_storage_assets" => "test",
    ...
    # Mass-assignment invariant: sanitize_params_for! slices to visible_fields then permit!s,
    # at top level AND at nested depth. accepts_nested_attributes_for accepts more than one shape
    # for the submitted params, so a crafted request can flip the cardinality (e.g. send an array
    # to a has_many that the config expects as {idx => {…}}); the sanitizer must still drop unallowed
    # keys for every shape, so a future "cleanup to params.permit" can't drift which keys survive.
    context "against cardinality-flip mass-assignment" do
      before do
        ActionController::Parameters.permit_all_parameters = false
      end
      ...
      it "drops a crafted unallowed key from a correctly-shaped nested record" do
        controller.params = ActionController::Parameters.new(
          "field_test" => {
            "nested_field_tests_attributes" => {
              "new_1" => {"title" => "ok", "unallowed_field" => "EVIL"}
            }
          }
        )
        controller.send(:sanitize_params_for!, :create, RailsAdminNext.config(FieldTest), controller.params["field_test"])
        nested = controller.params["field_test"]["nested_field_tests_attributes"]["new_1"]
        expect(nested.to_h).to eq("title" => "ok")
        expect(nested.permitted?).to be_truthy
      end

      it "drops unallowed keys when a has_many nested param is flipped to an array" do
        controller.params = ActionController::Parameters.new(
          "field_test" => {
            "nested_field_tests_attributes" => [
              {"title" => "ok", "unallowed_field" => "EVIL"}
            ]
          }
        )
        controller.send(:sanitize_params_for!, :create, RailsAdminNext.config(FieldTest), controller.params["field_test"])
      ...
      it "drops unallowed keys when a singular nested param is flipped to an array, without raising" do
        controller.params = ActionController::Parameters.new(
          "field_test" => {
            "comment_attributes" => [
              {"created_at" => "2010-08-04T00:00:00", "unallowed_field" => "EVIL"}
            ]
          }
        )
        RailsAdminNext.config Comment do
        ...
        record = controller.params["field_test"]["comment_attributes"].first
        expect(record.to_h.keys).not_to include("unallowed_field")
      end
    end
  end
