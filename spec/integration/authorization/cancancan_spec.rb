...
RSpec.describe "RailsAdminNext CanCanCan Authorization", type: :request do
  subject { page }

  before do
    # The CanCanCan adapter's default ability_class resolves top-level ::Ability at
    # request time, so this per-example stub is what `authorize_with(:cancancan)` picks up.
    ...
    stub_const("AdminAbility", Class.new do
      include CanCan::Ability
