...
RSpec.describe "Filtering multi-select widget", type: :request, js: true do
  ...
  context "when orderable" do
    ...
    it "reorders the selection and announces the move" do
      # The hidden select's server-rendered option order follows the list
      # default sort (id descending), so add in that order to keep the
      # visible selection and the hidden select in sync before moving.
      find(".ra-multiselect-collection option", text: /Leonys/).select_option
      within(".ra-multiselect-center") { click_link "Add new" }
      find(".ra-multiselect-collection option", text: /Cory/).select_option
      within(".ra-multiselect-center") { click_link "Add new" }
      expect(all("#team_player_ids option", visible: false).select(&:selected?).map(&:value)).to eq [players[1].id.to_s, players[0].id.to_s]

      find(".ra-multiselect-selection option", text: /Cory/).select_option
      within(".ra-multiselect-center") { click_link "Up" }

      expect(all("#team_player_ids option", visible: false).select(&:selected?).map(&:value)).to eq [players[0].id.to_s, players[1].id.to_s]
      expect(find('.ra-multiselect [role="status"]', visible: :all).text(:all)).to include "moved"
    end
  end
  ...
  context "when using remote requests" do
    ...
    context "with an already-selected record" do
      before do
        players[0].update! team: team
        visit edit_path(model_name: "team", id: team.id)
      end

      it "does not reappear in remote search results" do
        find("input.ra-multiselect-search").set("Cory")
        page.execute_script("document.querySelector('input.ra-multiselect-search').dispatchEvent(new KeyboardEvent('keydown'))")
        is_expected.to have_content "No objects found"
        expect(all(".ra-multiselect-collection option").map(&:text)).not_to include "Cory Burns"
        expect(all(".ra-multiselect-selection option").map(&:text)).to include "Cory Burns"
      end
    end

    it "ends up showing results only for the most recently typed term" do
      search = find("input.ra-multiselect-search")
      search.set("Cory")
      search.set("Leo")
      search.set("Matt")
      is_expected.to have_content "Matt Garza"
      expect(all(".ra-multiselect-collection option").map(&:text)).to match_array ["Matt Garza"]
    end
  ...
  it "announces selection changes in an aria-live region" do
    visit new_path(model_name: "team")
    expect(page).to have_css('.ra-multiselect [role="status"][aria-live="polite"]', visible: :all)
    find(".ra-multiselect-collection option", text: /Cory/).select_option
    within(".ra-multiselect-center") { click_link "Add new" }
    expect(find('.ra-multiselect [role="status"]', visible: :all).text(:all)).to include "selected"
  end
    ...
    before do
      ...
      RailsAdminNext.config Fan do
        field :division, :enum do
          enum { Division.pluck(:name, :custom_id).to_h }
          def value
            nil
          end
