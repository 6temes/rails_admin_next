...
RSpec.describe "New action", type: :request do
  ...
  describe "page" do
    ...
    it "does not use the 'wildcard route'" do
      is_expected.to have_no_selector("a[href^='/rails_admin_next/main/new']")
    end
  end
  ...
  context 'on create after a blocked "add another" click', js: true do
    before do
      visit new_path(model_name: "player")

      fill_in "player[number]", with: "42"
      fill_in "player[position]", with: "Second baseman"
      # Name is left blank: native required validation blocks this submit, but
      # the clicked button's hidden `_add_another` param is already appended
      # to the form and must not survive into the next, different submit.
      find_button("Save and add another").trigger("click")
    ...
    it "redirects to the index, not back to the new-record form" do
      is_expected.to have_text "Player successfully created"
      expect(page.current_path).to eq(index_path(model_name: "player"))
    end
  end
