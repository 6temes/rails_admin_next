...
RSpec.describe "Dashboard action", type: :request do
  ...
  it "renders the hardcoded Rails AdminNext brand in the navbar and title" do
    visit dashboard_path
    is_expected.to have_selector(".navbar-brand", text: "Rails AdminNext")
    is_expected.to have_title(/Rails AdminNext/)
  end

  it "shows statistics by default" do
    allow(RailsAdminNext.config(Player).abstract_model).to receive(:count).and_return(0)
    expect(RailsAdminNext.config(Player).abstract_model).to receive(:count)
    visit dashboard_path
  end
  ...
  it "does not show statistics if turned off" do
    ...
    expect(RailsAdminNext.config(Player).abstract_model).not_to receive(:count)
    visit dashboard_path
  end
  ...
  it "counts are different for same-named models in different modules" do
    allow(RailsAdminNext.config(User::Confirmed).abstract_model).to receive(:count).and_return(10)
    allow(RailsAdminNext.config(Comment::Confirmed).abstract_model).to receive(:count).and_return(0)
