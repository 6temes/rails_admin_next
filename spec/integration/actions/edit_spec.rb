...
RSpec.describe "Edit action", type: :request do
  ...
  describe "field grouping" do
    ...
    describe "help" do
      before do
        RailsAdminNext.config.included_models = [HelpTest, Team]
      end
      ...
      context "using active_record" do
      ...
      it "shows help section if present" do
        RailsAdminNext.config HelpTest do
          edit do
            group :default do
  ...
  context "with a readonly object" do
    ...
    it "raises ActionNotAllowed" do
      expect { visit edit_path(model_name: "read_only_comment", id: comment.id) }.to raise_error "RailsAdminNext::ActionNotAllowed"
    end
  end
  ...
  describe "add another" do
    before do
      ...
      @player = RailsAdminNext::AbstractModel.new("Player").first
    end
  ...
  context "with a submit button with custom value", js: true do
    before do
      ...
      execute_script %{var b = document.querySelector('.form-actions [name="_save"]'); b.setAttribute('name', 'player[name]'); b.setAttribute('value', 'Jackie Robinson');}
  ...
  context "with a field with 'format' as a name (conflicts with Kernel#format)" do
    it "is updatable without any error" do
      ...
      click_button "Save" # first(:button, "Save").click
      @record = RailsAdminNext::AbstractModel.new("FieldTest").first
  ...
  context "with composite primary keys", composite_primary_keys: true do
    ...
    context "using custom serializer" do
      before do
        RailsAdminNext.config.composite_keys_serializer = Class.new do
