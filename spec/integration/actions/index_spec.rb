...
RSpec.describe "Index action", type: :request do
  ...
  describe "page" do
    it 'shows "List of Models", should show filters and should show column headers' do
      RailsAdminNext.config.default_items_per_page = 1
      ...
      is_expected.to have_no_selector("a[href^='/rails_admin_next/main/list']")
    end
  end
  ...
  context "with pagination" do
    ...
    describe "with limited_pagination=false" do
      before { RailsAdminNext.config.default_items_per_page = 1 }
    ...
    context "with limited_pagination=true" do
      before do
        RailsAdminNext.config.default_items_per_page = 1
        allow(RailsAdminNext::AbstractModel.new(Player).config.list)
    ...
    describe "with an invalid per param" do
      it "clamps per=0 instead of raising FloatDomainError" do
        get index_path(model_name: "player", per: 0)
        expect(response).to have_http_status(:ok)
      end

      it "clamps a negative per instead of issuing a negative LIMIT" do
        get index_path(model_name: "player", per: -5)
        expect(response).to have_http_status(:ok)
      end
    end
  end
