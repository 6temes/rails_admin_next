...
RSpec.describe "HasManyAssociation field", type: :request do
  ...
  describe "on create" do
    ...
    it "creates an object with correct associations" do
      ...
      @league = RailsAdminNext::AbstractModel.new("League").all.to_a.last
      @divisions[0].reload
  ...
  context "with custom primary_key option" do
    ...
    before do
      RailsAdminNext.config.included_models = [ManagingUser, ManagedTeam]
      RailsAdminNext.config ManagingUser do
        field :teams
      end
    end
  ...
  context "with composite foreign keys", composite_primary_keys: true do
    ...
    describe "via default field" do
      ...
      context "with invalid key" do
        before do
          allow_any_instance_of(RailsAdminNext::Config::Fields::Types::HasManyAssociation)
    ...
    describe "via nested field" do
      ...
      before do
        RailsAdminNext.config NestedFan do
          field :name
          field :fanships
        end
      end
