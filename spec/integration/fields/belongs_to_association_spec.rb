...
RSpec.describe "BelongsToAssociation field", type: :request do
  ...
  context "with custom primary_key option" do
    ...
    before do
      RailsAdminNext.config.included_models = [ManagedTeam, ManagingUser]
      RailsAdminNext.config ManagedTeam do
        field :user
      end
    end
  ...
  context "with composite foreign keys", composite_primary_keys: true do
    ...
    describe "via default field" do
      ...
      context "with invalid key" do
        before do
          allow_any_instance_of(RailsAdminNext::Config::Fields::Types::BelongsToAssociation)
    ...
    describe "via remote-sourced field" do
      before do
        RailsAdminNext.config FavoritePlayer do
          field :fanship do
      ...
      it "allows update", js: true do
        ...
        expect(page).to have_selector('[role="option"]')
        find('[role="option"]', text: "Fanship ##{fanship.id}").click
        click_button "Save"
        is_expected.to have_content "Favorite player successfully updated"
        expect(FavoritePlayer.all.map(&:fanship)).to eq [fanship]
      end
