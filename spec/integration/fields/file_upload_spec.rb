...
RSpec.describe "FileUpload field", type: :request do
  ...
  it "updates the preview to the last selected file after re-selecting", js: true do
    visit new_path(model_name: "field_test")
    attach_file "String field", file_path("test.jpg")
    is_expected.to have_selector('#field_test_string_field_field img.preview[src^="data:image/jpeg"]')
    attach_file "String field", file_path("test.png")
    is_expected.to have_selector('#field_test_string_field_field img.preview[src^="data:image/png"]', count: 1)
  end
end
