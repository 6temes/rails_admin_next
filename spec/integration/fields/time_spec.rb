...
RSpec.describe "Time field", type: :request, active_record: true do
  ...
  context "on update" do
    ...
    it "updates the value" do
      ...
      expect(find('[name="field_test[time_field]"]', visible: false).value).to eq "03:45:00"
      find('[name="field_test[time_field]"]', visible: false).set("04:55:00")
      click_button "Save"
      field_test.reload
  ...
  context "with server timezone changed" do
    ...
    it "treats the time set by the browser as local time" do
      visit edit_path(model_name: "field_test", id: field_test.id)
      find('[name="field_test[time_field]"]', visible: false).set("04:55:00")
      click_button "Save"
      field_test.reload
    ...
    it "is not altered by just saving untouched" do
      visit edit_path(model_name: "field_test", id: field_test.id)
      expect(find('[name="field_test[time_field]"]', visible: false).value).to eq "00:45:00"
      click_button "Save"
